    ...
    if(ncfs_all%num_totalvl > 0) then
        do k=1,ncfs_all%numvar
           if(ncfs_all%num_dim(k) == 3) then
              if(ncfs_all%dim_1(k) <= 10) then
                ! treat small 3d as 2D field
                nlvl3d_small=nlvl3d_small+ncfs_all%dim_1(k)
              else
                nlvl3d=nlvl3d+1
              endif
           endif
           if(ncfs_all%num_dim(k) == 2) nlvl2d=nlvl2d+1
           if(nlvlmax < ncfs_all%dim_1(k)) nlvlmax=ncfs_all%dim_1(k)
        enddo
    endif
...
!
    ...
    write(6,'(2a5,2x,a10,10a10)') "core","fid","varname","vartype","nx","lvlbegin","lvlend"
    do k=1,ntotalcore
       write(6,'(2I5,2x,a10,10I10)') k,fileid(k),trim(varname(k)),vartype(k),nx(k),lvlbegin(k),lvlend(k) 
    enddo
    write(6,*) "======================================================================="
!
!  save results
