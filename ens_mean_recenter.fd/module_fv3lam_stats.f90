...
!    
       ...
       do kk=1,this%numvarfile(n)
          ...
          write(*,*) trim(name),ndim,dim_id(1:ndim)
          write(*,*) (dim_len(dim_id(i)),i=1,ndim)
          if(ndim==4) then
    ...
    this%num_totalvl=0 
    write(6,'(a20,10a10)') "variable_name","num_dim","nx","nz","data_type"
    do k=1,this%numvar
       this%num_totalvl=this%num_totalvl+this%dim_1(k)
       write(6,'(a20,10I10)') trim(this%list_varname(k)),this%num_dim(k),this%dim_2(k), &
                     this%dim_1(k),this%vartype(k)
    enddo
