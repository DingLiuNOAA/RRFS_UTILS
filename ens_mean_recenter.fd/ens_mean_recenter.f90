...
! decide control file name and ensemble mean file name
  do i=1,totalnumfiles
     input_control_file(i) = trim(adjustl(filebase))//'_control'//trim(filetail(i))
     input_ensmean_file(i) = trim(adjustl(filebase))//'_mean'//trim(filetail(i))
  enddo
  if(l_recenter) then
     do i=1,totalnumfiles
...
!
  ...
  if (MPI_COMM_NULL /= new_comm) then
     if(mype_vartype==5) then
        allocate(tmpd3r4(1,mype_nx,mype_ny))
     elseif(mype_vartype==6) then
        allocate(tmpd3r8(1,mype_nx,mype_ny))
     else
...
!
     ...
     do iens=0,ens_size
        ...
        if(iens==0) then
           ...
           filename = trim(adjustl(filebase))//'_mem'//memnun//trim(filetail(mype_fileid))
        endif
        !iret=nf90_open(trim(filename),nf90_nowrite,ncioid,comm=new_comm,info=MPI_INFO_NULL)
        iret=nf90_open(trim(filename),nf90_nowrite,ncioid)
...
! write mean
     if( l_write_mean ) then
        ...
        do ilev=mype_lbegin,mype_lend
           startloc=(/ilev,1,1/)
           countloc=(/1,mype_nx,mype_ny/)

           iret=nf90_inq_varid(ncioid,trim(adjustl(mype_varname)),var_id)
           if(mype_vartype==5) then
              tmpd3r4(1,:,1)=d3r8_mean(:,1,ilev)
              iret=nf90_put_var(ncioid,var_id,tmpd3r4,start=startloc,count=countloc)
           elseif(mype_vartype==6) then
              tmpd3r8(1,:,1)=d3r8_mean(:,1,ilev)
...
! get ensemble perturbations and then add it to the base state
        l_positive=.false.
        if( trim(mype_varname)=="qv" .or. trim(mype_varname)=="qr" .or.  &
           trim(mype_varname)=="qi" )then
           l_positive=.true.
        endif
     ...
     do iens=1,ens_size
        do ilev=mype_lbegin,mype_lend
           d4r4(:,1,ilev,iens)=d4r4(:,1,ilev,iens)-d3r8_mean(:,1,ilev)+d4r4(:,1,ilev,0)
           if(l_positive) d4r4(:,1,ilev,iens)=max(d4r4(:,1,ilev,iens), 0.0)
        enddo
     enddo
!
...
!
        do iens=1,ens_size
           ...
           filename = trim(adjustl(filebase))//'_mem'//memnun//trim(filetail(mype_fileid))
           !iret=nf90_open(trim(filename),nf90_write,ncioid,comm=new_comm,info=MPI_INFO_NULL)
           iret=nf90_open(trim(filename),nf90_write,ncioid)
           if(iret/=nf90_noerr) then
              write(6,*)' problem opening ', trim(filename),' fileid=',mype_fileid,', Status =',iret
              write(6,*)  nf90_strerror(iret)
              call flush(6)
           ...
           do ilev=mype_lbegin,mype_lend
              ...
              iret=nf90_inq_varid(ncioid,trim(adjustl(mype_varname)),var_id)
              if(mype_vartype==5) then
                 tmpd3r4(1,:,1)=d4r4(:,1,ilev,iens)
                 iret=nf90_put_var(ncioid,var_id,tmpd3r4,start=startloc,count=countloc)
              elseif(mype_vartype==6) then
                 tmpd3r8(1,:,1)=d4r4(:,1,ilev,iens)
                 iret=nf90_put_var(ncioid,var_id,tmpd3r8,start=startloc,count=countloc)
              endif
           enddo  ! ilev
