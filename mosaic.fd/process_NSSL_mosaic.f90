...
!
!  grid
  integer(i_kind) :: nlon,nlat,nCells
  real,allocatable:: xlon(:,:),ylat(:,:)
  real*8,allocatable:: cell_lon(:),cell_lat(:)    !
...
!
  ...
  namelist/setup/ analysis_time,dataPath
...
!
  mypeLocal=mype+1
  call readref%init(mypeLocal,datapath)
!
...
!
     ...
     call rrfs%get_dim("nCells",nCells)
     allocate(cell_lon(nCells))
     allocate(cell_lat(nCells))
     call rrfs%get_var("latCell",nCells,cell_lat)
     call rrfs%get_var("lonCell",nCells,cell_lon)
     rad2deg=180.0/3.1415926
     cell_lon=cell_lon*rad2deg
